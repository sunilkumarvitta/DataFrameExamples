...
object Ex9_SqlLiterals {
  def main(args: Array[String]): Unit = {
    ...
    postsDF.printSchema()
    postsDF.createOrReplaceTempView("posts_vw")
    sparkSession.sql("select * from posts_vw").show()
    sparkSession.sql("select authorID, subforum, count(*) from posts_vw group by authorID, subforum order by subforum, count(*) desc").show()
